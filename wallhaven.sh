#!/usr/bin/env bash
# Jus de Patate - 2020-2021 - jusdepatate@protonmail.com (github:jusdepatate)
...
# Explained: curl has argument `s` so we don't have a loading bar, sed tries to output all link with class "preview" and goes to wallhaven.cc/w/, head gives only the first line, `echo` is empty so that we have a new line.
...
if [ "$(curl -s "$png" | head -n1 | grep "404" > /dev/null || fatal "Couldn't check if $png gives 404 or not")" ]; then
	url="$jpg"
	type="jpg"
elif [ "$(curl -s "$jpg" | head -n1 | grep "404" > /dev/null || fatal "Couldn't check if $jpg gives 404 or not")" ]; then
	url="$png"
	type="png"
fi
...
if [ ! "$XDG_PICTURES_DIR" == "" ]; then
  file="$XDG_PICTURES_DIR/wallhaven/$id.$type"
  folder="$XDG_PICTURES_DIR/wallhaven/"
else
